...
from adata.common.utils.sunrequests import SunProxy, set_rate_limit, set_global_rate_limit
...
def rate_limit(limit: int = 30, domain: str = None):
    """
    设置请求频率限制
    :param limit: 每分钟允许的请求次数，默认：30
    :param domain: 要限制的域名，若不指定则设置全局默认值
    """
    if domain:
        set_rate_limit(domain, limit)
    else:
        set_global_rate_limit(limit)
    return
